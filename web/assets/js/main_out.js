(function () {
  ...
  function drawLeaderboard() {
    ...
    canvas.width = 200;
    canvas.height =
      leaderboard.type !== "pie" ? 60 + 24 * leaderboard.items.length : 240;
  ...
  function drawGame() {
    ...
    const gameStatsText =
      `${~~stats.fps} FPS` +
      (isNaN(stats.latency) ? "" : ` ${stats.latency}ms ping`);
  ...
  function keydown(event) {
    ...
    if (key === "enter") {
      ...
      if (key === " ") wsSend(UINT8_CACHE[minionControlled ? 22 : 17]);
      if (key === "q") {
  ...
  function init() {
    ...
    // const exitButton = byId("exitButton");
    // const exitModal = byId("exitModal");
    // const countdownElement = byId("countdown");

    // exitButton.addEventListener("click", () => {
    //   exitModal.style.display = "block"; // Відображаємо модальне вікно
    //   let countdown = 10;
    ...
    //       window.location.href = "/logout"; // Переадресація на сторінку виходу
    //     }
    //   }, 1000);
    // });
    ...
    const exitConfirmButton = document.getElementById("exitConfirm");
    let countdown;

    // Обробник натискання на кнопку виходу
    exitButton.addEventListener("click", function () {
      exitModal.style.display = "block"; // Показуємо модальне вікно

      let seconds = 10; // Лічильник часу
      countdown = setInterval(function () {
        seconds--;
        countdownElement.textContent = seconds; // Оновлюємо відлік на екрані

        if (seconds <= 0) {
          clearInterval(countdown); // Зупиняємо лічильник
          alert("Гра завершена!"); // Виводимо повідомлення
          window.location.reload(); // Перезавантажуємо сторінку або можемо виконати інші дії
        }
      }, 1000); // Інтервал в 1 секунду
    });

    // Обробник підтвердження виходу
    exitConfirmButton.addEventListener("click", function () {
      clearInterval(countdown); // Зупиняємо лічильник
      alert("Гра завершена!"); // Повідомлення про завершення
      window.location.reload(); // Перезавантажуємо сторінку або можемо виконати інші дії
    });
