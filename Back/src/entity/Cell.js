...
class Cell {
  ...
  onEat(prey) {
    if (prey.type === 4) {
      // Перевірка на спеціальну їжу
      this.setSize(this.radius * 2); // Подвоїти розмір клітини
    } else {
      if (!this.server.config.playerBotGrow) {
        if (this.radius >= 250 && prey.radius <= 41 && prey.type == 0)
          prey._radius2 = 0; // Не дозволяється рости з клітинами, які мають маленький розмір
      }
      this.setSize(Math.sqrt(this._radius2 + prey._radius2)); // Збільшити розмір для іншої їжі
    }
    // Важливо! Можна викликати onRemove або іншу обробку їжі
    prey.onRemove(this.server); // Видалити їжу після того, як її поглинуто
  }
